...
impl GmailOAuthService {
    ...
    pub fn get_auth_url(&self, state: &str) -> Result<(String, String)> {
        // Google OAuth 端点
        let auth_url_str = "https://accounts.google.com/o/oauth2/v2/auth".to_string();
        let token_url_str = "https://oauth2.googleapis.com/token".to_string();
    ...
    /// 交换授权码获取 token
    pub async fn exchange_code(&self, code: &str, state: &str) -> Result<GmailTokenResponse> {
        // Google OAuth 端点
        let token_url_str = "https://oauth2.googleapis.com/token".to_string();
        ...
        request_params.insert("code_verifier".to_string(), code_verifier);
    ...
    /// 刷新 access token
    pub async fn refresh_token(&self, refresh_token: &str) -> Result<GmailTokenResponse> {
        ...
        let token_response: GmailTokenResponse = response
            .json()
            .await
            .map_err(|e| MailError::Internal(format!("解析 token 响应失败: {}", e)))?;
...
mod tests {
    ...
    #[test]
    fn test_gmail_oauth_service_empty_client_id() {
        let service = GmailOAuthService::new(
            "".to_string(),
            "postium-mail://oauth/callback".to_string(),
            vec![],
    ...
    #[test]
    fn test_gmail_token_response_expiring() {
        // 即将过期的 token（300 秒 = 5 分钟）
        let expiring = GmailTokenResponse {
        ...
        // 未过期的 token
        let not_expiring = GmailTokenResponse {
            access_token: "test".to_string(),
            token_type: "Bearer".to_string(),
    ...
    #[test]
    fn test_gmail_token_json() {
        let original = GmailTokenResponse {
        ...
        let parsed = GmailTokenResponse::from_json(&json).unwrap();
    ...
    #[test]
    fn test_gmail_token_to_xoauth2() {
        ...
        let xoauth2 = token.to_xoauth2("user@gmail.com");
        ...
        assert!(decoded_str.contains("user=user@gmail.com"));
