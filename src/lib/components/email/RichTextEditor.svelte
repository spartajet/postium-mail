...
<!--
  ...
  使用 :global() 修改 Tiptap 编辑器的全局样式。
  因为 Tiptap 创建的 DOM 元素不在组件的 DOM 范围内（非 Shadow DOM），
  需要使用 :global() 选择器才能选中它们。

  样式说明：
  1. .tiptap - Tiptap 编辑器的根元素类名
     - outline: none：移除默认焦点轮廓，使用自定义样式
     - min-height: 100%：编辑器最小高度占满父容器

  2. .tiptap p.is-editor-empty:first-child::before - 占位符伪元素
     - 仅在编辑器内容为空时显示
     - content: attr(data-placeholder)：读取 Tiptap 设置的 data-placeholder 属性值
     - float: left：使占位符文本不占用文档流空间
     - color: muted-foreground：使用 CSS 变量设置灰色文字
     - pointer-events: none：禁止鼠标事件（不可选中占位符文本）
     - height: 0：不占用高度
-->
<style>
    /* Tiptap 编辑器根元素样式 */
    :global(.tiptap) {
        /* 移除浏览器默认的焦点轮廓 */
        outline: none;
        /* 编辑器最小高度占满父容器，确保点击空白区域也能聚焦 */
        min-height: 100%;
    }

    /* 占位符文本样式：当编辑器内容为空时显示提示文字 */
    :global(.tiptap p.is-editor-empty:first-child::before) {
        /* 从 data-placeholder 属性读取占位符文本内容 */
