<script lang="ts">
  ...
  import { RefreshCw, CircleX, CircleCheck, Globe, Sun, Moon } from 'lucide-svelte';
  ...
  function getProgressText(): string {
    ...
    switch (p.stage) {
      ...
      case 'SyncingFolders': return t.sync.syncingFolders;
      case 'SyncingEmails': return `${t.sync.syncingEmails}${p.folder ? ` (${p.folder})` : ''}`;
      case 'Completed': return t.sync.completed;
...
<div class="status-bar relative flex h-8 items-center justify-between border-t border-border bg-elevated/80 px-6 text-xs text-muted-foreground backdrop-blur-md">
  <div class="flex items-center gap-6">
    ...
    {:else if syncStatus === 'error'}
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-destructive">{syncStore.error || t.sync.failed}</span>
      </div>
    {:else}
      <div class="flex items-center gap-1.5">
