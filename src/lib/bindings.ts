/**
 * Postium Mail - Tauri 后端命令与类型绑定文件
 * bindings.ts
 *
 * ⚠️ 警告：此文件由 Tauri Specta 自动生成，请勿手动编辑！
 * 任何手动修改将在下次代码生成时被覆盖。
 *
 * ==================== 文件说明 ====================
 * 本文件包含前端与 Tauri Rust 后端之间的通信绑定，包括：
 *
 * 1. commands - 命令对象
 *    封装了所有可调用后端 Tauri 命令的异步方法。
 *    每个命令对应后端（Rust）中的一个 #[tauri::command] 函数，
 *    通过 invoke() 机制实现前后端通信。
 *
 *    主要命令分类：
 *    - 账号管理：listAccounts, getAccount, createAccount, updateAccount, deleteAccount
 *    - 邮件操作：listEmails, getEmail, searchEmails, markAsRead, toggleStar, deleteEmails
 *    - 邮件发送：sendEmail
 *    - 同步功能：syncAccount, getFolderStats
 *    - 服务商检测：detectProvider, listProviders
 *    - OAuth2 认证：startOauth2, pollOauth2
 *    - 标签管理：listLabels, createLabel, updateLabel, deleteLabel
 *    - 邮件标签关联：addLabelToEmail, removeLabelFromEmail, getLabelsForEmail
 *
 * 2. events - 事件对象
 *    封装了后端向前端推送的事件，用于实时通信。
 *    当前包含：
 *    - syncProgressEvent：同步进度事件，实时推送邮件同步的阶段进度
 *
 * 3. TypeScript 类型定义
 *    与后端 Rust 结构体一一对应的数据传输对象（DTO）类型，
 *    确保前后端数据结构的类型安全。
 *
 *    主要类型：
 *    - AccountDto：账号信息
 *    - EmailDto / EmailDetail：邮件信息
 *    - SyncProgress：同步进度
 *    - LabelDto：标签信息
 *    - ProviderInfo：邮件服务商信息
 *    - 其他请求/响应类型
 *
 * ==================== 生成方式 ====================
 * 此文件通过 Tauri Specta 工具从 Rust 后端代码自动生成。
 * 当后端 Rust 代码中的命令或类型发生变更时，需要重新运行生成命令
 * 以更新此文件。
 *
 * ==================== 使用方式 ====================
 * ```typescript
 * import { commands, events } from '$lib/bindings';
 * import type { AccountDto, SyncProgress } from '$lib/bindings';
 *
 * // 调用后端命令
 ...
 *   console.log('同步进度:', event.payload.progress);
 * });
 * ```
 */
...
export const commands = {
   ...
   */
  listAccounts: () =>
    typedError<AccountDto[], MailError>(__TAURI_INVOKE("list_accounts")),
  /**
   *  获取指定账号详情
   *
   *  根据账号ID获取单个账号的详细信息。
  ...
  getAccount: (id: number) =>
    typedError<AccountDto, MailError>(__TAURI_INVOKE("get_account", { id })),
  /**
   *  创建新账号
   *
   *  添加一个新的邮箱账号到应用中。
  ...
  createAccount: (request: CreateAccountRequest) =>
    typedError<AccountDto, MailError>(
      __TAURI_INVOKE("create_account", { request }),
    ),
  /**
   *  更新账号信息
   *
  ...
  updateAccount: (request: UpdateAccountRequest) =>
    typedError<AccountDto, MailError>(
      __TAURI_INVOKE("update_account", { request }),
    ),
  /**
   *  删除账号
   *
  ...
  deleteAccount: (id: number) =>
    typedError<null, MailError>(__TAURI_INVOKE("delete_account", { id })),
  /**
   *  获取邮件列表
   *
  ...
  listEmails: (
    ...
    typedError<EmailListResponse, MailError>(
      __TAURI_INVOKE("list_emails", { accountId, folder, page, limit }),
    ),
  /**
   *  按分类获取邮件列表
   *
  ...
  listEmailsByCategory: (
    ...
    typedError<EmailListResponse, MailError>(
      __TAURI_INVOKE("list_emails_by_category", {
        accountId,
        category,
        page,
        limit,
   ...
   */
  getEmail: (id: number) =>
    typedError<EmailDetail, MailError>(__TAURI_INVOKE("get_email", { id })),
  /**
   *  搜索邮件
   *
   *  根据关键词全文搜索邮件。
  ...
  searchEmails: (
    ...
    typedError<SearchResult[], MailError>(
      __TAURI_INVOKE("search_emails", { query, accountId, limit }),
    ),
  /**
   *  标记邮件已读/未读状态
   *
   ...
   *  ```
   */
  markAsRead: (emailId: number, isRead: boolean) =>
    typedError<null, MailError>(
      __TAURI_INVOKE("mark_as_read", { emailId, isRead }),
    ),
  /**
   *  切换邮件星标状态
   *
  ...
  toggleStar: (emailId: number) =>
    typedError<boolean, MailError>(__TAURI_INVOKE("toggle_star", { emailId })),
  /**
   *  删除邮件
   *
   *  删除一封或多封邮件。
  ...
  deleteEmails: (emailIds: number[]) =>
    typedError<number, MailError>(
      __TAURI_INVOKE("delete_emails", { emailIds }),
    ),
  /**
   *  移动邮件到指定文件夹
   *
   ...
   *  ```
   */
  moveEmailToFolder: (emailId: number, folder: string) =>
    typedError<null, MailError>(
      __TAURI_INVOKE("move_email_to_folder", { emailId, folder }),
    ),
  /**
   *  发送邮件
   *
  ...
  sendEmail: (request: SendEmailRequest) =>
    typedError<string, MailError>(__TAURI_INVOKE("send_email", { request })),
  syncAccount: (accountId: number) =>
    typedError<null, MailError>(__TAURI_INVOKE("sync_account", { accountId })),
  getFolderStats: (accountId: number) =>
    typedError<FolderStat[], MailError>(
      __TAURI_INVOKE("get_folder_stats", { accountId }),
    ),
  /**
   *  检测邮箱服务商
   *
  ...
  detectProvider: (email: string) =>
    typedError<ProviderDetectionResult, MailError>(
      __TAURI_INVOKE("detect_provider", { email }),
    ),
  /**
   *  列出所有邮箱服务商
   *
   ...
   */
  listProviders: () =>
    typedError<ProviderInfo[], MailError>(__TAURI_INVOKE("list_providers")),
  /**
   *  启动OAuth2授权流程
   *
  ...
  startOauth2: (
    ...
    typedError<OAuth2AuthUrl, MailError>(
      __TAURI_INVOKE("start_oauth2", { providerId, email, displayName }),
    ),
  /**
   *  轮询OAuth2授权状态
   *
   ...
   *  ```
   */
  pollOauth2: (state: string) =>
    typedError<OAuth2PollResult, MailError>(
      __TAURI_INVOKE("poll_oauth2", { state }),
    ),
  /**
   *  更新账号密码
   *
  ...
  updateAccountPassword: (id: number, password: string) =>
    typedError<null, MailError>(
      __TAURI_INVOKE("update_account_password", { id, password }),
    ),
  /**
   *  列出所有标签
   *
   ...
   *  ```
   */
  listLabels: (accountId: number) =>
    typedError<LabelDto[], MailError>(
      __TAURI_INVOKE("list_labels", { accountId }),
    ),
  /**
   *  创建新标签
   *
   ...
   *  ```
   */
  createLabel: (request: CreateLabelRequest) =>
    typedError<LabelDto, MailError>(
      __TAURI_INVOKE("create_label", { request }),
    ),
  /**
   *  更新标签信息
   *
   ...
   *  ```
   */
  updateLabel: (id: number, request: UpdateLabelRequest) =>
    typedError<LabelDto, MailError>(
      __TAURI_INVOKE("update_label", { id, request }),
    ),
  /**
   *  删除标签
   *
  ...
  deleteLabel: (id: number) =>
    typedError<null, MailError>(__TAURI_INVOKE("delete_label", { id })),
  /**
   *  为邮件添加标签
   *
   *  为指定邮件添加一个标签。
  ...
  addLabelToEmail: (emailId: number, labelId: number) =>
    typedError<null, MailError>(
      __TAURI_INVOKE("add_label_to_email", { emailId, labelId }),
    ),
  /**
   *  从邮件移除标签
   *
  ...
  removeLabelFromEmail: (emailId: number, labelId: number) =>
    typedError<null, MailError>(
      __TAURI_INVOKE("remove_label_from_email", { emailId, labelId }),
    ),
  /**
   *  获取邮件的所有标签
   *
   ...
   *  ```
   */
  getLabelsForEmail: (emailId: number) =>
    typedError<LabelDto[], MailError>(
      __TAURI_INVOKE("get_labels_for_email", { emailId }),
    ),
  /**
   *  按标签列出邮件
   *
   ...
   *  ```
   */
  listEmailsByLabel: (labelId: number) =>
    typedError<number[], MailError>(
      __TAURI_INVOKE("list_emails_by_label", { labelId }),
    ),
};
...
export type MailError =
  | { type: "AccountNotFound"; message: number }
  | { type: "AuthFailed"; message: string }
  | { type: "ImapConnectionFailed"; message: string }
  | { type: "SmtpSendFailed"; message: string }
  | { type: "SyncFailed"; message: string }
  | { type: "DatabaseError"; message: string }
  | { type: "KeyringError"; message: string }
  | { type: "ProviderNotSupported"; message: string }
  | { type: "InvalidParam"; message: string }
  | { type: "EmailNotFound"; message: number }
  | { type: "FolderNotFound"; message: string }
  | { type: "OAuthError"; message: string }
  | { type: "InvalidProvider"; message: string }
  | { type: "OAuth2Error"; message: string }
  | { type: "NotImplemented"; message: string }
  | { type: "LabelNotFound"; message: number }
  | { type: "ImapFolderMetadataFailed"; message: string }
  | { type: "ImapSearchFailed"; message: string }
  | { type: "BatchFetchHeadersFailed"; message: string }
  | { type: "ImapError"; message: string }
  | { type: "EmailMissingUid" };
